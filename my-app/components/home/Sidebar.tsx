...
const BASE_DATA: any = {
  ...
  nakliye: { open: 2500, unit: 50, unitLabel: 'km', label: 'Nakliye (Yurt İçi)', icon: <Truck size={18}/>, color: 'text-purple-600 bg-purple-100/80', group: 'NAKLİYE' },
  yurt_disi_nakliye: { open: 15000, unit: 150, unitLabel: 'km', label: 'Yurt Dışı Lojistik', icon: <Globe size={18}/>, color: 'text-indigo-600 bg-indigo-100/80', group: 'NAKLİYE' },
  seyyar_sarj: { open: 800, unit: 30, unitLabel: 'kw', label: 'Gezici Şarj', icon: <Zap size={18}/>, color: 'text-blue-600 bg-blue-100/80', group: 'ENERJİ' },
  istasyon: { open: 0, unit: 15, unitLabel: 'kw', label: 'Şarj İstasyonu', icon: <Navigation size={18}/>, color: 'text-blue-800 bg-blue-200/80', group: 'ENERJİ' },
  minibus: { open: 1500, unit: 35, unitLabel: 'km', label: 'Minibüs', icon: <Users size={18}/>, color: 'text-teal-600 bg-teal-100/80', group: 'YOLCU' },
  otobus: { open: 4000, unit: 80, unitLabel: 'km', label: 'Otobüs', icon: <Bus size={18}/>, color: 'text-teal-800 bg-teal-200/80', group: 'YOLCU' },
  vip_tasima: { open: 2500, unit: 60, unitLabel: 'km', label: 'VIP Transfer', icon: <Crown size={18}/>, color: 'text-emerald-700 bg-emerald-100/80', group: 'YOLCU' }
};
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-[85%] max-w-[360px] z-[2001] shadow-2xl border-r border-white/50 bg-white/25 backdrop-blur-[45px] transform transition-transform duration-700 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 space-y-8 custom-scrollbar pb-10">
          {activeTab === 'tariff' && (
            <>
              <div className="p-4 rounded-3xl bg-red-50/80 border border-red-200 flex items-start gap-3 shadow-sm">
                ...
                <p className="text-[9px] font-black text-red-700 leading-tight uppercase">Kazancımızın bir kısmı yardım kuruluşlarına aktarılmaktadır.</p>
              </div>
          ...
          {activeTab === 'history' && (
            <div className="space-y-4 animate-in slide-in-from-right-4 text-gray-900">
               {loading ? (
                  ...
                  orders.map((order) => (
                    <div key={order._id} className="rounded-[2rem] border-2 border-white/80 bg-white/60 backdrop-blur-md p-5 shadow-sm transition-all hover:bg-white active:scale-[0.98]">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <div className="text-right">
                           <p className="text-[14px] font-black text-blue-900">₺{order.totalPrice || '---'}</p>
                        </div>
                      </div>
                    </div>
                  ))
        ...
        <div className="p-8 bg-white/40 border-t border-white/20 text-center shrink-0">
          ...
          <div className="flex items-center gap-2 justify-center opacity-40 mt-4">
            <ShieldCheck size={14} />
            <span className="text-[9px] font-black text-gray-500 uppercase tracking-[0.4em]">Transport 245</span>
          </div>
        </div>
      </div>
