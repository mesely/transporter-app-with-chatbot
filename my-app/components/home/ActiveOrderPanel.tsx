...
export default function ActiveOrderPanel({ activeOrder, onComplete, onCancel }: ActiveOrderPanelProps) {
  ...
  const checkOrderStatus = async () => {
    ...
    try {
      const res = await fetch(`${process.env.BACKEND_URL || 'https://transporter-app-with-chatbot.onrender.com'}/orders/${activeOrder._id}`);
  ...
  const handleCustomerConfirm = async () => {
    ...
    try {
        await fetch(`${process.env.BACKEND_URL || 'https://transporter-app-with-chatbot.onrender.com'}/orders/${activeOrder._id}/status`, {
            method: 'PATCH',
