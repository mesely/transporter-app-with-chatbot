 ...
 * FIX: listContainerRef hatası ve TS any tiplemeleri giderildi.
 */
...
const SERVICE_OPTIONS = [
  ...
  { id: 'seyyar_sarj', label: 'GEZİCİ ŞARJ', icon: Zap, color: 'bg-cyan-500', subs: [] },
];
...
export default function ProviderModule() {
  ...
  const [formData, setFormData] = useState<any>({
    businessName: '', email: '', phoneNumber: '', city: 'İstanbul', district: '', address: '',
  ...
  const toggleServiceType = (id: string) => {
    setFormData((prev: any) => {
      ...
      let newTypes = isSelected ? [] : [id]; // Tekil seçim mantığı
      let newTags = isSelected ? [] : [...prev.filterTags];
  ...
  const handleSave = async () => {
    ...
    if (!phoneRegex.test(formData.phoneNumber)) return alert("Telefon 0 ile başlamalı ve 11 haneli olmalıdır.");
    if (formData.serviceTypes.length === 0) return alert("Lütfen bir hizmet branşı seçin.");
    ...
    try {
      ...
      let mainType = 'NAKLIYE'; // Default

      // 🔥 mainType Mapping
      if (['oto_kurtarma', 'vinc'].includes(subType)) mainType = 'KURTARICI';
      else if (['istasyon', 'seyyar_sarj'].includes(subType)) mainType = 'SARJ';
      else if (['yolcu_tasima', 'minibus', 'otobus', 'midibus', 'vip_tasima'].includes(subType)) mainType = 'YOLCU';
      else if (['tir', 'kamyon', 'kamyonet', 'evden_eve', 'yurt_disi_nakliye'].includes(subType)) mainType = 'NAKLIYE';
      ...
      const payload = {
        ...
        role: 'provider',
        mainType: mainType,
        serviceType: subType, // API uyumluluğu için subType'ı buraya da yazıyoruz
        pricing: {
    ...
    <div className="w-full min-h-screen p-6 space-y-8 bg-slate-50 selection:bg-blue-100">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex gap-3 w-full md:w-auto">
          {selectedProviders.length > 0 && (
            <button onClick={handleBulkDelete} className="flex-1 md:flex-none bg-red-600 text-white px-6 py-4 rounded-[2rem] text-xs font-black uppercase flex items-center justify-center gap-2 shadow-xl shadow-red-500/20">
              <Trash2 size={18} /> ({selectedProviders.length}) Sil
            </button>
          )}
          <button 
            onClick={() => { setIsEditing(false); setFormData({businessName:'', email:'', phoneNumber:'', city:'İstanbul', district:'', address:'', serviceTypes:[], openingFee:350, pricePerUnit:40, filterTags:[]}); setShowModal(true); }}
            className="flex-1 md:flex-none bg-slate-900 hover:bg-blue-600 text-white px-8 py-4 rounded-[2rem] text-xs font-black uppercase flex items-center justify-center gap-3 shadow-xl transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-2 flex items-center">
          ...
          <input placeholder="İsim veya Tel ile Ara..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-transparent p-3 font-bold text-xs uppercase outline-none"/>
        </div>
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-2 flex items-center">
      ...
      <div ref={listContainerRef} className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 pb-20">
        {loading ? (
          <div className="col-span-full flex flex-col items-center py-20"><Loader2 className="animate-spin text-blue-600" size={48}/></div>
        ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-14 h-14 ${ui.color} text-white rounded-2xl flex items-center justify-center shadow-lg`}>
                      <ui.icon size={28} strokeWidth={2}/>
                    </div>
                    <div>
                      <h3 className="font-black text-slate-800 text-base uppercase truncate max-w-[150px]">{p.businessName || p.firstName}</h3>
                      <div className="flex gap-1 mt-1 flex-wrap">
                        ...
                        {p.service?.tags?.slice(0,2).map((t:string) => <span key={t} className="text-[8px] font-black text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded uppercase">{t.replace('_',' ')}</span>)}
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2" onClick={e => e.stopPropagation()}>
                    <button onClick={() => openEdit(p)} className="p-2.5 bg-white text-slate-400 hover:text-blue-600 rounded-xl shadow-sm border border-slate-100 transition-colors"><Edit size={16}/></button>
                    <button onClick={(e) => handleDelete(e, p._id)} className="p-2.5 bg-white text-red-300 hover:text-red-600 rounded-xl shadow-sm border border-slate-100 transition-colors"><Trash2 size={16}/></button>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 text-[11px] font-bold text-slate-600 bg-slate-50 p-3 rounded-2xl"><MapPin size={14} className="text-red-500"/> {p.address?.city} / {p.address?.district || 'Merkez'}</div>
                </div>
              </div>
            )
          })
          ...
          <div className="bg-white w-full max-w-5xl h-[92vh] rounded-[3.5rem] p-8 md:p-12 shadow-2xl relative overflow-y-auto border border-white/50 custom-scrollbar">
            ...
            <header className="mb-10">
              <h2 className="text-4xl font-black text-slate-800 uppercase italic tracking-tighter">
                {isEditing ? 'Bilgileri Güncelle' : 'Yeni Kurum Ekle'}
              </h2>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
              ...
              <div className="space-y-6">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-600"></span> Kurumsal Detaylar</h3>
                <input placeholder="İŞLETME ADI" value={formData.businessName} onChange={e=>setFormData({...formData, businessName: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 font-black text-sm outline-none focus:bg-white focus:border-blue-400 transition-all"/>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <input placeholder="TELEFON (05...)" value={formData.phoneNumber} onChange={e=>setFormData({...formData, phoneNumber: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 font-bold text-xs outline-none"/>
                </div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <textarea placeholder="TAM ADRES VEYA DURAK BİLGİSİ..." value={formData.address} onChange={e=>setFormData({...formData, address: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 font-medium text-xs outline-none h-32 resize-none"/>
              </div>

              {/* Form Right */}
              <div className="space-y-6">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-600"></span> Hizmet Branşı</h3>
                      ...
                      <div key={opt.id} className="relative">
                        ...
                        {isSelected && hasSubs && (
                          <button 
                            ...
                            className="absolute -top-2 -right-2 bg-slate-900 text-white p-1.5 rounded-full shadow-lg z-10 animate-bounce"
                          >
                            <Settings2 size={14} />
                          </button>
                        )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                    <label className="text-[8px] font-black text-slate-400 uppercase block mb-1">Açılış (₺)</label>
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                    <label className="text-[8px] font-black text-slate-400 uppercase block mb-1">Birim (₺)</label>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 min-h-[80px] p-4 bg-slate-50 border border-slate-200 rounded-3xl">
                    {formData.filterTags.map((tag: string) => (
                      <span key={tag} className="flex items-center gap-1 bg-slate-800 text-white px-3 py-1.5 rounded-xl text-[9px] font-black uppercase shadow-md">
                        {tag.replace('_',' ')} <button onClick={() => setFormData({...formData, filterTags: formData.filterTags.filter((t:string)=>t!==tag)})} className="hover:text-red-400 ml-1 transition-colors"><X size={12}/></button>
                      </span>
                    ))}
                    {formData.filterTags.length === 0 && <span className="text-[10px] text-slate-300 italic uppercase">Alt özellik seçilmedi...</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 pt-10 border-t border-slate-100 flex justify-end">
              <button 
                onClick={handleSave} 
                disabled={loading}
                className="bg-slate-900 text-white py-6 px-16 rounded-[2.5rem] font-black uppercase text-sm tracking-[0.3em] shadow-2xl active:scale-95 flex items-center gap-3 transition-all disabled:opacity-50"
              >
                {loading ? <Loader2 className="animate-spin" size={24}/> : <>{isEditing ? 'GÜNCELLEMEYİ KAYDET' : 'KAYDI TAMAMLA'} <ArrowRight size={20}/></>}
              </button>
            </div>
          </div>
        </div>
      ...
      {activeFolder && currentFolderConfig && (
        <div className="fixed inset-0 z-[10000] flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="relative w-full sm:max-w-xl bg-white rounded-t-[3rem] sm:rounded-[3rem] p-8 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-2xl font-black uppercase italic text-slate-800 leading-tight">{currentFolderConfig.label}</h2>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Özellikleri Seçiniz (Çoklu Seçim)</p>
              </div>
              <button onClick={() => setActiveFolder(null)} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 shadow-sm transition-all hover:bg-red-500 hover:text-white"><X size={20} /></button>
            </div>
              ...
              {currentFolderConfig.subs.map(sub => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-center justify-center py-8 rounded-[2rem] transition-all border-2 ${isTagSelected ? `${currentFolderConfig.color} text-white shadow-xl scale-[1.02]` : 'border-slate-100 bg-slate-50 text-slate-400 hover:border-slate-300'}`}
                  >
            ...
            <button onClick={() => setActiveFolder(null)} className="mt-6 w-full py-5 rounded-[2rem] font-black uppercase text-xs shadow-xl text-white bg-slate-900 transition-all hover:bg-black active:scale-95">SEÇİMİ BİTİR</button>
          </div>
        </div>
      )}
    </div>
