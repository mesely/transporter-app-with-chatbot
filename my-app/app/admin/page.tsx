    ...
    return (
      <div className="fixed inset-0 bg-[#f1f5f9] flex items-center justify-center p-6 z-[9999]">
        ...
        <div className="w-full max-w-sm bg-white/70 backdrop-blur-3xl rounded-[3.5rem] shadow-2xl p-12 border border-white relative z-10 flex flex-col items-center">
          <div className="w-20 h-20 bg-gray-900 text-white rounded-[2rem] flex items-center justify-center mb-8">
            <Lock size={30} strokeWidth={1.5} />
          </div>
          <h1 className="text-2xl font-black text-gray-900 uppercase tracking-tighter mb-8 text-center">Yönetici Girişi</h1>
          <form onSubmit={handleLogin} className="w-full space-y-4">
            <input 
              type="password" placeholder="Erişim Şifresi" autoFocus value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full bg-white border border-gray-100 rounded-2xl p-4 text-center font-black text-lg outline-none focus:ring-4 focus:ring-blue-500/10 transition-all shadow-inner"
            />
            <button className="w-full bg-gray-900 text-white py-4 rounded-[1.2rem] font-black text-[10px] uppercase tracking-[0.25em] active:scale-95 transition-all">
              Panele Bağlan
            </button>
          </form>
        </div>
        ...
        <header className="h-20 bg-white/40 backdrop-blur-md border-b border-gray-200 px-8 flex items-center justify-between shrink-0 z-40">
          ...
          <div className="flex items-center gap-4 ml-auto">
             <div className="text-right hidden sm:block">
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">Sistem Yöneticisi</p>
                <p className="text-xs font-black text-gray-900 uppercase mt-1">Cloud Ops v2.4</p>
             </div>
             <div className="w-12 h-12 bg-white rounded-xl border border-gray-100 flex items-center justify-center text-gray-400 relative">
                <UserCircle size={24} />
                <div className="absolute top-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full"></div>
             </div>
             <button className="p-3 bg-gray-50 text-gray-400 rounded-xl hover:text-blue-600 transition-all"><Settings size={18} /></button>
          </div>
        </header>
